...
function vetur(options: Options, list: NormalizeData[]): {
  tags: Tags
  attributes: Attributes
} {
  const { prefix, reComponentName, attributesProp, attributesType, attributesDescription, attributesOptions, separator } = options
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (!title) continue
    const name = prefix || '' + (typeof reComponentName === 'function' ? reComponentName(title, fileName, path) : hyphenate(title))
    ...
    _attributes.forEach(item => {
      ...
      if (_item) {
        ...
        const options = item[attributesOptions].split(separator).map(item => item.trim())
